...
import { HeaderWrapper, HeaderTitle, LinksWrapper, LinkItem,
CurrentLinkItem } from './HeaderStyle';
...
const Header = () => {
  ...
  return (
    <HeaderWrapper>
      ...
      <LinksWrapper>
      {location.pathname === '/' ? 
      <CurrentLinkItem key='rec' onClick={() => navigateTo('/')}>RECENT</CurrentLinkItem> :
      ...
      {menuLinks.map((link) => {
        ...
        return (
          location.pathname.slice(1, 4) === `${endpoint.slice(0, 3)}` ? (
            <CurrentLinkItem key={endpoint.slice(0, 3)} onClick={() => navigateTo(endpoint)}>
              {link}
            </CurrentLinkItem>
          ) : (
            <LinkItem
